...
def predict_qualification(found_skills, all_features):
    ...
    try:
        ...
        # Strictly align input features with model training order
        input_dict = {feature: (1 if feature in found_skills else 0) for feature in all_features}
        input_data = pd.DataFrame([input_dict])
        
        # Reorder columns if model has feature_names_in_ attribute
    ...
    with st.spinner('Processing your profile...'):
        ...
        if resume_text.strip():
            ...
            if missing:
                ...
                st.success("Your profile perfectly aligns with this role's requirements!")
        else:
