...
'.join([p.text for p in docx.Document(file_obj).paragraphs])
        ...
        if resume_text and not resume_text.startswith('Error'):
            ...
            with c2:
                ...
                st.write("**Matched:** " + ", ".join([f"✅ {s.title()}" for s in sorted(list(matched))]) if matched else "None")
                st.write("**Missing:** " + ", ".join([f"❌ {s.title()}" for s in sorted(list(missing))]) if missing else "None")
