...
<style>
    ...
    .resource-card { background-color: #ffffff; border: 1px solid #e1e4e8; border-radius: 8px; padding: 10px; margin-bottom: 10px; }
</style>
...
# Sidebar - Removed emojis from keys and labels to prevent UnicodeEncodeError
...
if uploaded_file and analyze_btn:
    ...
    st.divider()
    st.subheader("Detailed Learning Roadmap")
    if missing:
        st.info(f"We identified {len(missing)} key skill gaps. Follow the resources below to upskill.")
        for s in sorted(missing):
            with st.container():
                st.markdown(f"#### Master **{s.title()}**")
                res = SKILL_RESOURCES.get(s, {})
                col1, col2, col3, col4 = st.columns(4)
                col1.markdown(f"[YouTube]({res.get('youtube', '#')})")
                col2.markdown(f"[Coursera]({res.get('coursera', '#')})")
                col3.markdown(f"[Udemy]({res.get('udemy', '#')})")
                col4.markdown(f"[Official Docs]({res.get('documentation', '#')})")
                st.markdown("--- ")
    else: st.success("Your profile is a perfect match!")
else: st.info("Please upload a resume in the sidebar and click 'Start Analysis'.")
